...
axios.interceptors.request.use((config: any) => {
  ...
  if (config.method === 'post') {
    if (config.data && JSON.parse(config.data).requestName) {
      requestName = JSON.parse(config.data).requestName
    } else {
...
Vue.prototype.$get = (url: any, headers: any, params: any) => {
  return new Promise((resolve, reject) => {
    axios.get(url, { headers: headers, params: params }).then((res: any) => {
